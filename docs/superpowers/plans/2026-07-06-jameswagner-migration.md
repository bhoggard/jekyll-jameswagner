...
**Tasks 1-10 are complete.** The site is live at `https://jameswagner.pages.dev` (Cloudflare Pages project `jameswagner`, account "Tristan Media"). Domain/DNS: user explicitly chose NOT to move jameswagner.com to Cloudflare yet (still on Opalstack DNS) — the pages.dev subdomain is the only live URL for now.

**Post-Task-10 follow-up work, done 2026-07-09** (small fixes discovered via ad-hoc audit, not part of the original numbered tasks):
- Commit `e10eda2` (pushed to `origin/main`): audited every numeric `/mt_archives/NNNNNN.html` URL referenced *internally* from within jameswagner.com's own post bodies (523 unique IDs found via DB scan) against the local backup's file coverage. Found 5 referenced IDs with no backup file, of which 2 (entry 5332 "alyson_shotz_", entry 6052 "noah_lyon_at_ho_1") are published with valid canonical targets but were being silently skipped by `tools/generate-jameswagner-legacy-redirects.rb` (it only scans `SOURCE_BACKUP_DIR`). Added an explicit `EXTRA_IDS_MISSING_BACKUP_FILE` allowlist to that script so these — and any future cases found the same way — keep generating stubs; regenerated `mt_archives/005332.html` and `mt_archives/006052.html`. Also fixed `_posts/2007-10-03-second_annual_n.md`, which still linked to entry 6052 via a raw, unrewritten, and doubly-prefixed (`http://http://`) legacy URL instead of its canonical permalink.
- Commit `d8c4fce` (**local only, not yet pushed** as of this writing): committed `tools/migrate-mt-image-map.yml` and `tools/scan-mojibake.py` into the repo at the user's request. **This reverses the "intentionally never committed" decision recorded below** — that section is now historical/inaccurate for those two files; see the correction inline.
- The other 3 IDs found in that same audit (entry 2727 "the_antichelsea", entry 4872 "are_we_finally", entry 6576 "scot_kaplan_par_1") are referenced via old numeric URLs but belong to **unpublished drafts** (`entry_status = 1`) that were never migrated — no canonical page exists to redirect to. Left as-is; user said to leave them in draft status. If their old numeric URLs need to keep working too, those drafts would need to be published/migrated first — not done, not decided either way.
...
**Correction (2026-07-09): the two files below are no longer uncommitted.** This paragraph originally documented `tools/migrate-mt-image-map.yml` and `tools/scan-mojibake.py` as intentionally excluded from git, matching bloggy's precedent. The user later asked for both to be committed (`d8c4fce`, local-only as of this writing — see status note above), reversing that decision. Kept here for history: the image map is a regenerable byproduct of `tools/migrate-mt-to-jekyll.rb` (re-run it to regenerate if ever needed); `scan-mojibake.py` is scratch tooling for the known encoding exceptions above (detection: cp1252-misread-as-UTF-8 signature `[Ãâ][continuation-bytes]+`, including 5 undefined-in-cp1252 byte values 0x81/0x8D/0x8F/0x90/0x9D via Latin-1 passthrough; fix: iteratively reverse `.encode('cp1252').decode('utf-8')` up to 3 passes).
